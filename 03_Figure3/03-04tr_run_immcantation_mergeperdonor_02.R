...
writeChangeoDb(merge_file, file = file.path(opt$outdir, paste0(donor_id, "_ig", opt$savename)))

# TR merge
tr_files <- which(read_file$vdj_type == "tr")
...
for (i in tr_files){
    ...
    merge_file_tr <- rbind(merge_file_tr, temp)
}

message(capture.output((table(as.factor(merge_file_tr$sample_id)))))
message(capture.output((table(as.factor(merge_file_tr$locus)))))

writeChangeoDb(merge_file_tr, file = file.path(opt$outdir, paste0(donor_id, "_tr", opt$savename)))
