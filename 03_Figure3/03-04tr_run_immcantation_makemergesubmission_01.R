...
option_list <- list(
    make_option(c("--infile"), default="/well/legacy/users/pps914/output/005_year1multi/005_year1multi_immarch/01032024_immcantationsubmissionfile.tsv",
  	...
  	make_option(c("--outdir"), default="/well/legacy/users/pps914/output/005_year1multi/005_year1multi_immarch/04-2_MergebyDonor_TR",
    ...
    make_option(c("--fileextension"), default="_db-pass_cleaned_gexonly",
...
for (i in 1:length(donor_ids)){
  ...
  # Library Output
  data_dir_temp <- ifelse(dat_temp$vdj_type == "ig", paste0(opt$filelocations, dat_temp$sample_id, "_", dat_temp$vdj_type, opt$fileextension, ".tsv"), paste0(opt$filelocations, dat_temp$sample_id, "_", dat_temp$vdj_type, opt$fileextension, "_data.tsv"))
    
  out_librarycsv <- data.frame("donor_id" = dat_temp$donor_id, "sample_id" = dat_temp$sample_id, "vdj_type" = dat_temp$vdj_type, "data_dir" = data_dir_temp)
...
write.table(out_submissiontsv, file.path(opt$outdir, "04-2_MergeByDonor_submissionfile.tsv"), quote=FALSE, sep='\t', row.names = FALSE)